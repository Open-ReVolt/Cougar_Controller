...
typedef struct {
	...
	unsigned spares[6];					// space for future use
...
{
	#ifdef PWM8K
	strcpy_P(uart_str, PSTR("Cougar OS controller firmware v1.10 (8KPWM)\r\n"));
	#else
	strcpy_P(uart_str, PSTR("Cougar OS controller firmware v1.10\r\n"));
	#endif
	uart_putstr();
}
