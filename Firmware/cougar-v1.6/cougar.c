...
void pi_loop(void)
{
	...
	if ((throttle_counter & 0x03) == 0x00) {
		...
		if (config.battery_amps_limit > 0) {
			...
			if (ocr1a_lpf > 0) {
				// PWM > 0
				ocr32 = bat_amp_lim_510 / (unsigned long)ocr1a_lpf;
				if (ocr32 < MAX_CURRENT_REF) {
					uv2 = ocr32;
...
{
	#ifdef PWM8K
	strcpy_P(uart_str, PSTR("Cougar OS controller firmware v1.6 (8KPWM)\r\n"));
	#else
	strcpy_P(uart_str, PSTR("Cougar OS controller firmware v1.6\r\n"));
	#endif
	uart_putstr();
}
