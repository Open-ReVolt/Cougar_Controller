	...
	if ((throttle_counter & 0x03) == 0x00) {
		...
		if (config.motor_os_th > 0) {
			...
			if (fault_bits & MOTOR_OS_FAULT) {
				...
				luv1 = (unsigned long)ocr1a_ghost << 16;
				if (current_fb > 0) luv1 = luv1 / (unsigned long)current_fb;
				// if current_fb is 0, consider it to be one, so luv1 = (luv1 / 1), do nothing
...
{
	#ifdef PWM8K
	strcpy_P(uart_str, PSTR("Cougar OS controller firmware v1.7 (8KPWM)\r\n"));
	#else
	strcpy_P(uart_str, PSTR("Cougar OS controller firmware v1.7\r\n"));
	#endif
	uart_putstr();
}
