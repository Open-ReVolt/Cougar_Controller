...
// autocrc.h should only be included for Linux build environment
// for windows, the inclusion of autocrc.h must be commented out
#include "autocrc.h"
...
void pi_loop(void)
{
	...
	ocr32 = (unsigned long)uv1 << 16;
	switch ((unsigned char)config.pwm_filter) {
		case 0:
			//ocr1a_lpf = ((ocr1a_lpf * 127) + uv1) >> 7;
			ocr1a_lpf_32 = ((ocr1a_lpf_32 * 127) + ocr32) >> 7;
			break;
		case 1:
			//ocr1a_lpf = ((ocr1a_lpf * 63) + uv1) >> 6;
			ocr1a_lpf_32 = ((ocr1a_lpf_32 * 63) + ocr32) >> 6;
			break;
		case 2:
			//ocr1a_lpf = ((ocr1a_lpf * 31) + uv1) >> 5;
			ocr1a_lpf_32 = ((ocr1a_lpf_32 * 31) + ocr32) >> 5;
			break;
		case 3:
			//ocr1a_lpf = ((ocr1a_lpf * 15) + uv1) >> 4;
			ocr1a_lpf_32 = ((ocr1a_lpf_32 * 15) + ocr32) >> 4;
			break;
		default:
			ocr1a_lpf = 0;
	}
	...
	if ((throttle_counter & 0x03) == 0x00) {
		...
		if (config.battery_amps_limit > 0) {
			...
			if (ocr1a_lpf > config.battery_amps_limit) {
				// we can only exceed battery amps limit if pwm > battery_amps_limit
				uv2 = MAX_CURRENT_REF - (ocr1a_lpf - config.battery_amps_limit);
				// uv2 is now maximum motor current we can have at given PWM
				// to keep battery amps within limit
		...
		// run battery amps and hours logic at 1KHz, uv1 = OCR1A (PWM)
		// battery_amps = (current_fb * pwm) / 512;
		battery_amps = ((unsigned long)current_fb * (unsigned long)uv1) >> 8;
...
{
	#ifdef PWM8K
	strcpy_P(uart_str, PSTR("Cougar OS controller firmware v1.15 (8KPWM)\r\n"));
	#else
	strcpy_P(uart_str, PSTR("Cougar OS controller firmware v1.15\r\n"));
	#endif
	uart_putstr();
}
...
void show_config(void)
{
	...
	strcpy_P(uart_str, PSTR("motor_os_threshold=xxxx\r\n"));
	u16_to_str(&uart_str[19], config.motor_os_th, 4);
	uart_putstr();

	strcpy_P(uart_str, PSTR("motor_os_ftime=xxxx\r\n"));
	u16_to_str(&uart_str[15], config.motor_os_ft, 4);
	uart_putstr();
...
int main(void)
{
	...
	tm_show_data = tm_100 = tm_1 = get_time();
				...
				if (fault_bits & MOTOR_OS_FAULT) {
					...
					if (motor_os_fault_timer) {
						...
						if (motor_os_fault_timer == 0) {
							...
							cli(); fault_bits &= ~MOTOR_OS_FAULT; sei();
						}
					}
				}
				else {
					...
					v1 = (unsigned long)x << 16;
					if (rt_data.current_fb > 0) v2 = rt_data.current_fb;
					else v2 = 1;
					v1 = v1 / v2;
					if (v1 > motor_overspeed_threshold) {
						if (motor_os_count < 5) motor_os_count++;
						else {
