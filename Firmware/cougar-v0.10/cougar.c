...
void pi_loop(void)
{
	...
	switch ((unsigned char)config.pwm_filter) {
		case 0:
			ocr1a_lpf = ((ocr1a_lpf * 127) + (unsigned)OCR1A) >> 7;
			break;
		case 1:
			ocr1a_lpf = ((ocr1a_lpf * 63) + (unsigned)OCR1A) >> 6;
			break;
		case 2:
			ocr1a_lpf = ((ocr1a_lpf * 31) + (unsigned)OCR1A) >> 5;
			break;
		case 3:
...
void show_menu(void)
{
	strcpy_P(uart_str, PSTR("Cougar OS controller firmware v0.10\r\n"));
	uart_putstr();
}
	...
	while (1) {
		...
		if (config.rtd_period) {
			if (diff_time(tm_show_data) >= config.rtd_period) {
				...
				cli(); x = OCR1A; sei();
				strcpy_P(uart_str, PSTR("TR=xxx CR=xxx CF=xxx PW=xxx HS=xxxx RT=xxxx FB=xx\r\n"));
