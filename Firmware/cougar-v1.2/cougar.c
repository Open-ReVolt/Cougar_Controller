...
void show_menu(void)
{
	strcpy_P(uart_str, PSTR("Cougar OS controller firmware v1.2\r\n"));
	uart_putstr();
}
...
void show_config(void)
{
	...
	strcpy_P(uart_str, PSTR("motor_os_threshold=xxxxx\r\n"));
	u16_to_str(&uart_str[19], config.motor_os_th, 5);
	uart_putstr();

	strcpy_P(uart_str, PSTR("motor_os_ftime=xxxxx\r\n"));
	u16_to_str(&uart_str[15], config.motor_os_ft, 5);
	uart_putstr();
}
